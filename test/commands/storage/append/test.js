...
const { Append, Set } = require('../../../../domain/commands');
      ...
      describe('correct message returned', () => {
        ...
        describe('without noreply', () => {
          it(`should return ${NOT_STORED}`, () => {
            const append = new Append(firstOptions);
            append.options = append.convertDataArrayToObject();
            append.options = { ...append.options, value: firstValue };
            const actual = append.doStoreOperation(store);
        ...
        describe('with noreply', () => {
          it(`should return '${EMPTY_SPACE}'`, () => {
            const append = new Append(firstOptionsWithNoReply);
            append.options = append.convertDataArrayToObject();
            append.options = { ...append.options, value: firstValue };
            const actual = append.doStoreOperation(store);
    ...
    describe('existent key', () => {
      describe('correct object properties replaced', () => {
        before(() => {
          ...
          const append = new Append(secondOptions);
          append.options = append.convertDataArrayToObject();
          append.options = { ...append.options, value: secondValue };
          append.doStoreOperation(store);
        });
        describe('key', () => {
        ...
        describe('value', () => {
          it('should have appended the value property successfully', () => {
            const { value } = store.find(testKey);
            const actual = value;
            const expected = '22333';
            assert.strictEqual(actual, expected);
          });
        });
        describe('flags', () => {
